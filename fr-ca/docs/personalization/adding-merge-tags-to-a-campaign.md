...
1. Vous verrez maintenant le menu d'options Gérer les attributs de contact
   Lors de l'inscription, les champs par défaut (ID de contact, Courriel, Date d'inscription) sont automatiquement créés et ajoutés ici basés sur vos informations d'enregistrement de compte. Le système les utilise pour suivre les informations d'abonnés et ne peuvent pas être supprimés ou modifiés.
...
Vous pouvez aussi utiliser l'étiquette [courriel], qui prendra l'adresse courriel du destinataire et l'insérera dans la campagne :

```"Ce courriel a été envoyé à [courriel], comme demandé."```
deviendrait :
```"Ce courriel a été envoyé à courriel@domaine.com, comme demandé."```
...
```[SUBJECT] "Bonjour, Ce courriel concerne [SUBJECT]"```

#### Information d'expéditeur

L'information pour ces étiquettes provient des détails remplis dans la section compte.

```"Ce courriel a été envoyé par [SENDER_NAME]" [SENDER_EMAIL]```

```"Pour plus d'informations, veuillez envoyer un courriel à [SENDER_EMAIL]"```
...
Votre adresse physique doit apparaître dans tous les courriels envoyés via un fournisseur de services de marketing par courriel pour être conforme CAN SPAM.
...
Ces étiquettes peuvent être utilisées comme partie de votre pied de page de courriel pour fournir au destinataire l'information de qui leur envoie des courriels (ceci est obligatoire comme partie de nos Conditions d'utilisation) :
...
```"Vous recevez ce courriel car vous faites partie de notre liste [LISTNAME]"```

#### Actions

Ces étiquettes permettent aux lecteurs de partager votre campagne avec d'autres, visualiser le courriel dans une fenêtre de navigateur et utiliser plusieurs options d'abonnement et de désabonnement. Ces étiquettes peuvent être ajoutées comme partie de votre en-tête ou pied de page de courriel.
...
Utilisé dans un courriel où vous voulez permettre aux abonnés de visualiser une version HTML du courriel dans une fenêtre de navigateur. C'est souvent utile pour des situations où les images pourraient ne pas être affichées, ou les gens pourraient recevoir un courriel texte par défaut.
...
- Prenez votre lien de suivi Google Analytics, par exemple : http://www.mondomaine.com/?utmsource=newsletter&utmmedium=courriel&utm_campaign=nouvellesseptembre
- Pour suivre le trafic basé sur le nom d'entreprise, vous prendriez l'étiquette [CLIENTS.COMPANY_NAME] et l'inséreriez dans votre lien : http://www.domaine.com/?utmsource=**[CLIENTS.COMPANYNAME]**&utmmedium=courriel&utmcampaign=nouvellesseptembre
- Quand cette campagne sera envoyée, l'URL ressemblera à : http://www.domaine.com/?utmsource=MonNomEntreprise&utmmedium=courriel&utm_campaign=nouvellesseptembre
- Si vous voulez voir les sources de trafic du site par adresse courriel individuelle : http://www.domaine.com/?utmsource=[courriel]&utmmedium=courriel&utm_campaign=nouvellesseptembre
- Quand cette campagne sera envoyée, l'URL ressemblera à : http://www.domaine.com/?utmsource=bonjour@moncourriel.com&utmmedium=courriel&utm_campaign=nouvellesseptembre
...
- Utilisez la même syntaxe exacte que dans le corps du courriel
...
- [Utiliser les balises de fusion et solutions de repli](/fr-ca/docs/personalization/using-merge-tags-and-fallbacks) - Guide des balises de fusion
- [Ajouter des attributs à votre liste](/fr-ca/docs/audience/adding-attributes-to-your-list) - Créer des champs de fusion
- [Ajouter une balise de fusion de date](/fr-ca/docs/personalization/adding-a-date-merge-tag) - Formatage de date
- [Utiliser le concepteur de courriel](/fr-ca/docs/campaign-creation/using-the-email-designer) - Insérer des balises de fusion
- [Utiliser le contenu dynamique](/fr-ca/docs/personalization/using-dynamic-content) - Personnalisation avancée