...
Les variables sont les critères sur lesquels vous basez votre contenu. Cela peut inclure des champs de liste comme prenom, nom, ou tout autre champ de données que vos abonnés ont soumis du contenu. Cela peut aussi inclure l'activité de campagne, vous permettant d'envoyer du contenu spécifique seulement à ceux qui ont cliqué sur un lien ou ouvert une campagne de email précédente.
...
Nous pouvons aussi nous référer à l'action qu'un abonné a prise pour une campagne de email précédente incluant l'ouverture du message, ou cliquer sur tout lien dans le email lui-même. Pour se référer à une campagne spécifique, vous devrez trouver l'ID de mailing et vous y référer en utilisant MAILING(id), où id est une référence au numéro de campagne. Pour se référer à un lien spécifique, vous devrez trouver l'ID de lien et vous y référer en utilisant LINK(id), où id est une référence au numéro de lien.

Exemple : MAILING(43243) LINK(3245364)

Une campagne de email a deux statuts différents : CLICKED et OPENED Un lien peut être : CLICKED

### Comment trouver un id de campagne de email dans votre site de marque.
...
`email` LIKE "%hotmail%"
(leur adresse email contient le texte "hotmail")
...
(leur adresse email contient le texte "hotmail" ET ils ont ouvert la campagne #34342)
...
- [Personnaliser avec les conditions d'affichage](/fr/docs/personalization/display-conditions) - Affichage conditionnel
- [Ajouter des balises de fusion à une campagne](/fr/docs/personalization/adding-merge-tags-to-a-campaign) - Champs dynamiques
- [Ajouter un flux RSS](/fr/docs/campaign-creation/adding-rss-feeds) - RSS dynamique
- [Créer un segment](/fr/docs/audience/creating-dynamic-segments) - Segments dynamiques
- [Utiliser l'IA pour créer du contenu](/fr/docs/ai/creating-content-with-ai) - Contenu IA dynamique